...
//
int WebIf_IdleCb(Common_Definition_t *Common_Definition);
...
int8_t espconn_sent(Entry_WebIf_Definition_t* p_conn, uint8_t* p_send_buffer, uint16_t send_buffer_write_pos);
int8_t espconn_connect(Entry_WebIf_Definition_t* p_conn);
int8_t espconn_set_opt(Entry_WebIf_Definition_t* p_conn, uint8_t opt);
...
enum ConnCtrlFlags {
    F_GENERATE_IDLE_CALLBACK	= 1 << 0  // Force one further callback 
					  //  (may be by generating one idle callback)
  , F_CONN_IS_AUTHENTICATED	= 1 << 1  // this connection is authenticated
  , F_TXED_CALLBACK_PENDING	= 1 << 2  // data_sent_cb_pending something was sent. 	
					  //  Sent Callback is pending. Do not send again.  
...
// should by called from platform when an HTTPD connection is disconnected, to clean up
void HTTPD_Disconnected_Cb(void *arg);

//
void WebIf_ReconCb(void *arg, int8_t err);

//
void WebIf_IdleCbx(void *arg);
...
struct http_parser_settings_s
{
  ...
  http_cb      on_chunk_header;		// if on_chunk_header is called, 
					// the chunk length is stored in parser->content_length.
