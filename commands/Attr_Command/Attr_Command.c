...
ProvidedByCommand_t Attr_ProvidedByCommand = {
  ...
  4,						    // length of cmd
  Attr_Initialize_Command_Fn,	// Initialize Fn
 ...
 */
int
Attr_Initialize_Command_Fn(SCDERoot_t* p_SCDERoot_from_core)
{
  // make data root locally available
...
// --------------------------------------------------------------------------------------------------
  ...
  // take *, it is at 'attr_name' text start (seek-start-pos)
  attr_value.p_char = attr_name.p_char;

  // clear element seek-counter
  j = 0;

  // seek * to end of previous args text ('attr_name') (to next '\32' -> space)
  while( ( i < args.len ) && ( *attr_value.p_char != ' ' ) ) { i++ , j++ ; attr_value.p_char++ ; }

  // length of previous args text ('attr_name') determined
  attr_name.len = j;
